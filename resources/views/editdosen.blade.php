...
<body>
    ...
    <form class='mt-4' method="post" action="/dosen/updated/{{ $dsn->id }}"> 
    @csrf <!-- {{ csrf_field() }} --> 
    @method('PUT') <!-- {{ method_field('PUT') }} --> 
    @php $bhs = array(); 
    $bhs = explode(',', $dsn->bahasa); 
    @endphp 
    <div class="form-group"> 
    <label for="formGroupExampleInput">NIDN</label> 
    <input type="number" class="form-control" name="nidn" id="nidn" value="{{ $dsn->nidn }}" readonly> </div> 
    <div class="form-group"> 
    <label for="formGroupExampleInput">Nama Dosen</label> 
    <input type="text" class="form-control" name="nama" id="nama" value="{{ $dsn->nama }}"> </div> 
    <label for="formGroupExampleInput">Gender</label> 
    <div class="form-check"> 
    <input class="form-check-input" type="radio" name="gender" value="Laki-Laki" value="Laki-Laki" @php if (($dsn->gender)=='Laki-Laki') echo 'checked' @endphp> 
    <label class="form-check-label" for="exampleRadios1"> Laki-Laki </label> </div> 
    <div class="form-check"> 
    <input class="form-check-input" type="radio" name="gender" value="Perempuan" @php if (($dsn->gender)=='Perempuan') echo 'checked' @endphp> 
    ...
    <option value="Bandung" @php if (($dsn->kota)=='Bandung') echo 'selected' @endphp>Bandung</option> 
    <option value="Yogyakarta" @php if (($dsn->kota)=='Yogyakarta') echo 'selected' @endphp>Yogyakarta</option> </select> </div> 
